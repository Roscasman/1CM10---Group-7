...
#Empty lists to store the created PH vectors
list_PH_job_vectors = []
list_PH_machine_vectors = []
 
#To change the amount of PHs used, change the range in the for loop. Also change the first for loop of random initialization to the same digit as used below.
for x in range(10):
...
def Genetic_Algorithm(list_PH_job_vectors, list_PH_machine_vectors, Jobb_vector_init, machinelist, N_p):
    ...
    for x in list_job_vectors_parents:
        ...
        b=b+1
    if k==1:
        list_job_vectors_parents.append(list_PH_job_vectors[0]) #Add one normal PH chromosome
        list_machine_vectors_parents.append(list_PH_machine_vectors[0])
        for x in range(1, len(list_PH_job_vectors)): #Add 9 mutated PH chromosome
            mutated_job_vector = swap_mutation(list_PH_job_vectors[x])
            list_job_vectors_parents.append(mutated_job_vector)
            mutated_machine_vector = uniform_mutation(list_PH_machine_vectors[x])
            list_machine_vectors_parents.append(mutated_machine_vector)
